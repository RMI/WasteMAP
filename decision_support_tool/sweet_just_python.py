...
msw_per_capita_defaults = {'Australia and New Zealand' : 1.644,
                           'Caribbean' : 2.137,
                           'Central America' : 1.589,
                           'Eastern Africa' : .795,
                            'Eastern Asia' : 1.315,
                            'Eastern Europe' : 1.014,
                            'Middle Africa' : .521,
                            "North America" : 2.63,
                            "Northern Africa" : 1.123,
                            "Northern Europe" : 1.315,
                            "Rest of Oceania" : .932,
                            "South America" : 1.178,
                            "Central Asia" : .932,
                            "South-Eastern Asia" : 1.26,
                            "Southern Africa" : .904,
                            "Southern Europe" : 1.288,
                            "Western Africa" : 0.493,
                            "Western Asia" : 1.89,
                            "Western Europe" : 1.616,
                            "Southern Asia" : 1.37}
...
for row in cities.iterrows():
    ...
    region = region_lookup[row[1]['country_name']]
    ...
    # landfills
    landfill_sites = ['Landfill', 'Dump']
    mcfs = {'Landfill': mcf_landfill, 'Dump': mcf_dump}
    diversion_fractions = {'Landfill': np.nan_to_num(row[1]['waste_treatment_controlled_landfill_percent'])/100,
                            'Dump': (np.nan_to_num(row[1]['waste_treatment_open_dump_percent']) +
    ...
    if diversion_total == 0:
        diversion_fractions = {'Landfill': 0, 'Dump': 1}
    else:
    ...
    # compost
    compost_total = 0
    compost_components = set(['food', 'green', 'wood']) # Double check we don't want to include paper
    ...
    # anaerobic digestion
    anaerobic_total = 0
    components_anaerobic = set(['food', 'green', 'wood'])
    ...
    # Currently the non 100% split between landfills is a problem. 
    ...
    for site in landfill_sites:
        ...
        for waste in components:
            ...
            for t in range(50):
                ...
                ch4 = []
                for y in range(t):
                    val = ks[waste] * L_0[waste] * ms[mcf][waste][y] * np.exp(-ks[waste] * (t - y - 0.5)) * mcf
                    ch4.append(val)
                #Q3[waste][t] = convert_methane_m3_to_ton(sum(ch4)) * 28
                # if sum(ch4) == 0.0:
                #     break
                q[mcf][waste][t] = sum(ch4) #* .9
...
for i in diversion_fractions.keys():
    print(i, waste_total * diversion_fractions[i])

waste_total / population * 1000 / 365
sum([waste_total * diversion_fractions[x] for x in diversion_fractions.keys()]) / population * 1000 / 365

print(compost_fraction_of_total)
print(anaerobic_fraction_of_total)
print(combustion_fraction_of_total)

print(compost_total)
print(anaerobic_total)
print(combustion_total)

{x: waste_fractions[x] / fraction_recyclable for x in components_recycling}
sum([waste_fractions[x] / fraction_recyclable for x in components_recycling])
...
#runs_q[city][1] + runs_q[city][.4]
runs_m[city][1] + runs_m[city][.4]
# %%
